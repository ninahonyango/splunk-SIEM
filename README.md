...
The output indicates that the service was active and enabled, confirming that rsyslog has been successfully installed.
...
Now that the Windows host machine, Kali Linux VM and Metasploitable2 VM are all on the same network and configured to operate within the same subnet (192.168.56.0/24), connectivity was verified to ensure that they are communicating as expected. 
...
A new data input must be added in Splunk so as to collect and ingest external data for indexing, searching, monitoring, and analysis.

This is a core step in any Splunk setup because without data inputs, Splunk has nothing to analyze.
...
To enable remote logging on Metasploitable2, the syslogd config was edited Metasploitable2 terminal:
...
The file vsftpd.log holds logs generated by the vsftpd service. It is specifically used to record FTP server activity. It contains logs such as connection attempts, authentication failures, commands issued by the client/attacker (Kali VM), errors and warnings such as improper commands or syntax. 
...
I ran a failed login attempts' test exploit on the FTP service from Kali and verified that the vsftpd.log file in Metasploitable2 was receiving logs by running:
...
To test the vsftpd.log log forwarding configuration in step 6.4 above and to generate logs, activities were triggered on Metasploitable2 from Kali. 

I tried logging into the FTP server using fake username and password as illustrated below:

![Fake FTP Login Attempts Screenshot](images/bruteforce3.png)
*Screenshot on failed FTP login attempts from Kali*

I also performed a nmap scan to identify open ports and their corresponding services and versions by running:
...
To check for Metasploitable's ftp failed login logs and connection attempts from Kali, a new Kali terminal was launched and the following command executed:
...
From the screenshots above, we can see entries like connection attempts as a result of the nmap scan and authentication failures on vsftpd service, which confirmed that log forwarding and the attack simulation from the previous steps is working properly.
